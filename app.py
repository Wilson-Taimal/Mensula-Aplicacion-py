# ================================================================================================ #
# APLICACIÓN WEB - DISEÑO DE MÉNSULAS #
from flask import Flask
from flask import render_template, request, send_file       # para llamar las plantillas de html
...
def inicio():
    if request.method == 'POST':
        ...
        material.fc = float(request.form['fc'])
        material.fy = float(request.form['fy'])
        material.u = float(request.form['u'])
        material.phiv = float(request.form['phiv'])
        material.phif = float(request.form['phif'])

        # Numero de barras a usar
        ref.Nb = float(request.form['Nb'])
        ref.Nbe = float(request.form['Nbe'])

        # Geometría
        geom.av = float(request.form['av'])
        geom.b = float(request.form['b'])
        geom.c = float(request.form['c'])
        geom.h = float(request.form['h'])
        geom.r = float(request.form['r'])
        geom.bw = float(request.form['bw'])

        # Cargas de diseño
        carga.Vu = float(request.form['Vu'])
        carga.Nuc1 = float(request.form['Nu'])
        ...
        carga.Nuc2 = round(carga.Nuc2, 2)
        carga.Nuc = round(carga.Nuc, 2)
        carga.Mu = round(carga.Mu, 2)
        geom.hmin = round(geom.hmin, 2)
        calculo.Vc1 = round(calculo.Vc1, 2)
        calculo.Vc2 = round(calculo.Vc2, 2)
        calculo.Vc3 = round(calculo.Vc3, 2)
        calculo.Vc = round(calculo.Vc, 2)
        calculo.Avf = round(calculo.Avf, 2)
        calculo.An = round(calculo.An, 2)
        calculo.Af = round(calculo.Af, 2)
        calculo.As1 = round(calculo.As1, 2)
        calculo.As2 = round(calculo.As2, 2)
        calculo.As3 = round(calculo.As3, 2)
        calculo.As = round(calculo.As, 2)
        calculo.Ah = round(calculo.Ah, 2)
        calculo.Aa = round(calculo.Aa, 2)
        calculo.Ascol = round(calculo.Ascol, 2)
        calculo.pcal = round(calculo.pcal, 4)
        calculo.pmin = round(calculo.pmin, 4)
        calculo.lw = round(calculo.lw, 2)
        calculo.tw = round(calculo.tw, 2)
        ...
        valor = np.array(
            [V1, V2, V3, V4, V5, V6, V7, V8, V9, V10, V11, V12, V13, V14, V15, V16, V17, V18, V19, V20, V21, V22, V23,
             ...
             V45, V46, V47, V48, V49, V50, V51, V52, V53, V54, V55, V56, V57, V58, V59, V60, V61, V62, V63, V64, V65,
             V66, V67])
        unidad = np.array(
            [U1, U2, U3, U4, U5, U6, U7, U8, U9, U10, U11, U12, U13, U14, U15, U16, U17, U18, U19, U20, U21, U22, U23,
             U24, U25, U26, U27, U28, U29, U30, U31, U32, U33, U34, U35, U36, U37, U38, U39, U40, U41, U42, U43, U44,
             U45, U46, U47, U48, U49, U50, U51, U52, U53, U54, U55, U56, U57, U58, U59, U60, U61, U62, U63, U64, U65,
             U66, U67])
        ...
        excel_filename = "D:\APP_REPORTES\ResultadosMensula.xlsx"
        df.to_excel(excel_filename, index=False)

        return render_template('resultados.html', material = material, geom = geom, carga = carga, ref = ref, calculo = calculo, chequeo = chequeo, excel_filename=excel_filename)
